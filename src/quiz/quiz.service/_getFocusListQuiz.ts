...
async function _getFocusListQuiz(
  ...
  const { kind, userId, quizRecordListOfUser } = getListQuizInputType;

  const listIdQuizInRecordFailedSorted =
  ...
  const listIdQuizInRecordSolvedSorted =
    quizRecordListOfUser ||
    []
  ...
  const listIdQuizInRecordSorted = [
    ...listIdQuizInRecordFailedSorted,
    ...listIdQuizInRecordSolvedSorted,
  ];

  if (kind === KindGetFocusListQuiz.myQuizByRecord) {
    // 내 퀴즈들중  A(아직 시도해보지 않은 것들) + B(전에 틀린 것들 + 전에 맞은 것들)

    const result = await quizRepository.find({ userId });

    // A. 아직 시도해보지 않은 퀴즈들
    const listQuizNotInRecord = result.filter(
      (e) => !listIdQuizInRecordSorted.includes(e.id),
    );

    // 멤버 기록에 존재하는 퀴즈와, 실제로 존재하는 퀴즈들의 교집합
    const listIdQuizInRecordSortedExisting = listIdQuizInRecordSorted.filter(
      (e) => {
        const index = result.findIndex((e) =>
          listIdQuizInRecordSorted.includes(e.id),
        );
